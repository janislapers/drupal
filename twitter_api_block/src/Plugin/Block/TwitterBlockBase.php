...
class TwitterBlockBase extends BlockBase implements ContainerFactoryPluginInterface {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, StateInterface $state, KillSwitch $kill_switch, LoggerChannelFactory $logger) {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['options']['count'] = [
      ...
      '#default_value' => isset($config['options']['count']) ? $config['options']['count'] : self::DEFAULT_COUNT,
      '#required'      => TRUE,
    ];
  ...
  public function getTweets(string $url, string $parameters) {
    ...
    try {
      ...
      $this->logger->get('twitter')->error($e->getMessage());
    }
